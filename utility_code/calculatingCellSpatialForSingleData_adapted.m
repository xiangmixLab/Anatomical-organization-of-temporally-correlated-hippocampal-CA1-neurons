function [firingRateAll,countAll,countTime] = calculatingCellSpatialForSingleData_adapted(neuron,behavpos,behavtime,behavROI,binsize,Segment,threshold,temp,intensity,plotting,countTimeThresh)
%% Inputs:
...
%         thresh = (max(neuron.S(Segment(k),:))-min(neuron.S(Segment(k),:)))*threshold; % the threshold above which the neuron is active
        thresh = (max(neuron.S(Segment(k),:))-0)*threshold; % the threshold above which the neuron is active
        else
            thresh = threshold(Segment(k));
        end
...
%         thresh = (max(neuron.C(Segment(k),:))-min(neuron.C(Segment(k),:)))*threshold; % the threshold above which the neuron is active
        ...
        idx = find(neuron.C(Segment(k),:)>thresh);
    end
    if ~isempty(idx)
