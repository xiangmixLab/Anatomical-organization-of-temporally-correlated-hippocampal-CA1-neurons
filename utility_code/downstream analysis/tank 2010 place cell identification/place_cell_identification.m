...
function [place_cells]=place_cell_identification_tank2010(neuron,behavpos,behavtime,ROI,thresh)
...
for i=1:length(firingrate)
    ...
    % 2: place field should larger than 4*4 bins (2-3cm*2-3cm)
    potentialFields={};
    ...
    mean_rate=sum(nS>0)/(length(nS)/15);
