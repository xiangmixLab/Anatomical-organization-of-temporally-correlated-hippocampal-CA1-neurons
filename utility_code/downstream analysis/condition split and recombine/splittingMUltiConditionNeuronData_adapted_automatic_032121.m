function neuronIndividuals = splittingMUltiConditionNeuronData_adapted_automatic_032121(neuron,filename,mscamid,behavcamid,timestampname)
neuronIndividuals = cell(1,length(neuron.num2read)-1);
%     [FileName,PathName,FilterIndex] = uigetfile('.mat','select neuron data');
neuron.num2read(neuron.num2read==0)=[];
...
for i = 1:length(neuronIndividuals)
    ...
    if ~isempty(timestampname{i})
        fid=fopen(timestampname{i},'r');
        mscamindex=mscamid(i);
        behavindex=behavcamid(i);
