...
function [place_cells,all_infoScore,all_infoScore_norm,all_coherence] = permutingSpike_adapt_041222_LT(neuron,behavpos,behavtime,temp,occThresh,binsize,small_velo,infosign,coh_thresh)
...
for j = 1:length(firingrateAll)
    ...
    frt=zscore(firingrateAll{j});
    frt=frt-min(frt(:));
    
    if size(frt,1)<size(frt,2)
        frt=nanmean(frt,1);
    else
        frt=nanmean(frt,2);
    end
