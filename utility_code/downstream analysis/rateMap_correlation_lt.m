...
function rateMap_corr=rateMap_correlation_lt(fs1,fs2,ct1,ct2,resize_sign)
rateMap_corr=[];
ctt=1;
...
if ~isempty(fs2)
    for i=1:length(fs1)
        if ~isempty(fs1{i})&&~isempty(fs2{i})
            ...
            h=fspecial('gaussian',[1,5],2);
            cell1t=conv(cell1t,h,'same');
            cell2t=conv(cell2t,h,'same');
            if resize_sign==1
                cell2t=resample(cell2t,length(cell1t),length(cell2t));
                ct2=resample(ct2,length(ct1),length(ct2));
            end
