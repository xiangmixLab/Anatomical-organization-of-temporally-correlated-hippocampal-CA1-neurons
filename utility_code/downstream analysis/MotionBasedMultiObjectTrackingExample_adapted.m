...
function alltracks = MotionBasedMultiObjectTrackingExample_adapted(ai5,directo)
...
for pp=1:size(ai5,3)
    frame = single(squeeze(ai5(:,:,pp)));
    [area, centroids, bboxes, majoraxis, minoraxis, orientation, eccentricity, extent, perimeter, mask] = detectObjects(frame);
    ...
    displayTrackingResults([size(ai5,1) size(ai5,2)],directo);
    alltracks{1,ct}=tracks;
    ct=ct+1;
end
...
%         obj.reader = vision.VideoFileReader('dark_part_centers_new_corp01.avi');
...
%             'NumTrainingFrames', 40, 'MinimumBackgroundRatio', 0.7);
        ...
        obj.blobAnalyser = vision.BlobAnalysis('BoundingBoxOutputPort', true, ...
            'AreaOutputPort', true, 'CentroidOutputPort', true, 'MajorAxisLengthOutputPort', true,'MinorAxisLengthOutputPort', true,'OrientationOutputPort', true,'EccentricityOutputPort',true,'ExtentOutputPort',true,'PerimeterOutputPort',true, ...
            'MinimumBlobArea', 1);
    end
...
% remove noisy pixels and to fill the holes in the remaining blobs.  
    ...
    function [area, centroids, bboxes, majoraxis, minoraxis, orientation, eccentricity, extent, perimeter, mask] = detectObjects(frame)
...
%         mask = imopen(mask, strel('rectangle', [3,3]));
        ...
        [area, centroids, bboxes, majoraxis, minoraxis, orientation, eccentricity, extent, perimeter] = obj.blobAnalyser.step(mask);
    end
...
% current frame, and update its bounding box accordingly.
    ...
    function predictNewLocationsOfTracks()
        for i = 1:length(tracks)
            bbox = tracks(i).bbox;
%             areat= tracks(i).area;
%             majoraxist= tracks(i).majoraxis;
%             minoraxist= tracks(i).minoraxis;
%             orientationt= tracks(i).orientation;
%             eccentricityt= tracks(i).eccentricity;
%             extentt= tracks(i).extent;
%             perimetert= tracks(i).perimeter;
            % Predict the current location of the track.
            predictedCentroid = predict(tracks(i).kalmanFilter1);
            predictedorientation = predict(tracks(i).kalmanFilter2);
            % Shift the bounding box so that its center is at 
            % the predicted location.
            predictedCentroid = int32(predictedCentroid) - bbox(3:4) / 2;
            tracks(i).bbox = [predictedCentroid, bbox(3:4)];            
            tracks(i).orientation = predictedorientation;
%             tracks(i).majoraxis = majoraxist;
%             tracks(i).minoraxis = minoraxist;
%             tracks(i).orientation = orientationt;
%             tracks(i).eccentricity = eccentricityt;
%             tracks(i).extent = extentt;
%             tracks(i).perimeter = perimetert;
        end
    end
...
% indices of tracks and detections that remained unassigned. 
    ...
    function [assignments, unassignedTracks, unassignedDetections] = ...
        ...
        for i = 1:nTracks
%             cost(i, :) = distance(tracks(i).kalmanFilter1, centroids)+0.5*distance(tracks(i).kalmanFilter2, orientation);% larger cost means more unlikely be assigned to a current track
            cost(i, :) = distance(tracks(i).kalmanFilter1, centroids);
        end
...
% visible count by 1. Finally, the function sets the invisible count to 0. 
    ...
    function updateAssignedTracks()
        ...
        for i = 1:numAssignedTracks
            ...
            orientationt = orientation(detectionIdx, :);
            % Correct the estimate of the object's location
            % using the new detection.
            correct(tracks(trackIdx).kalmanFilter1, centroid);
            correct(tracks(trackIdx).kalmanFilter2, orientationt);
            
            % Replace predicted bounding box with detected
            % bounding box.
            tracks(trackIdx).bbox = bbox;
            tracks(trackIdx).orientation = orientationt;
...
% to eliminate noisy detections, such as size, location, or appearance.
    ...
    function createNewTracks()
        ...
        orientation = orientation(unassignedDetections, :);
        
        for i = 1:size(centroids, 1)
            
            centroid = centroids(i,:);
            bbox = bboxes(i, :);
            orientationt = orientation(i,:);
            % Create a Kalman filter object.
            kalmanFilter1 = configureKalmanFilter('ConstantVelocity', ...
                centroid, [30, 10], [30, 10], 10);%
            kalmanFilter2 = configureKalmanFilter('ConstantVelocity', ...
                orientationt, [30, 10], [30, 10], 10);
...
% displays the frame and the mask in their respective video players. 
    ...
    function displayTrackingResults(sz,sav)
        ...
        frame=frame(1:sz(1),1:sz(2));
        obj.maskPlayer.step(mask);        
        obj.videoPlayer.step(frame);
        
        if ~isempty(sav)
            mkdir([sav,'\','tracking illustration']);
            imwrite(frame,[sav,'\','tracking illustration\frame',num2str(ct),'.tif']);
        end
    end
