function [firingRateAll,countAll,countTimeAll,countTime,amplitudeAll,amplitude_rateAll,binInfo,countTime_pt] = calculatingCellSpatialForSingleData_Suoqin(neuron,behavpos,behavtime,behavROI,binsize,segments,threshold,temp,intensity,plotting,countTimeThresh,small_velo)
%% Inputs:
...
downsampling = round(length(neuron.time)/size(neuron.C,2));
if downsampling ~= 1
    %     downsampling == 2
...
while ts < length(behavtime)
    ...
    for j = ts+1:length(behavtime)
        ...
        if idxxj == idxxi & idyyj == idyyi
            ...
            countTime_pt(idxxi,idyyi) = countTime_pt(idxxi,idyyi)+1;
