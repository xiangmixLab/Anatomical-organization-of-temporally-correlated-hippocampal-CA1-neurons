function [ velocity ] = extract_velocity_adapted(interp_behav_vec, ca_time,sfactor)
...
elseif size(interp_behav_vec,2) == 2
    for i=2:length(ca_time)
        ...
        dis_list(i,:) = sqrt((interp_behav_vec(i,1)-interp_behav_vec(i-1,1)).^2 + (interp_behav_vec(i,2)-interp_behav_vec(i-1,2)).^2);
    end
...
velocity = smoothdata(velocity,'SmoothingFactor',sfactor);
