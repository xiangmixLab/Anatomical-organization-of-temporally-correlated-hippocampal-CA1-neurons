function [sumFiringRate_conv,sumFiringRateObject,sumFiringRate] = comparingFiringRateSingleConditionMultiObjects_050321(firingRate,countTime,objpos,binsize,behavROI)
            ...
            for u=-obj_range:obj_range % the obj is large, a larger area may be required (5x5 around obj)
                for v=-obj_range:obj_range
                    if bROI(4)-posObjects(i,2)+u>0&&bROI(4)-posObjects(i,2)+u<size(sumFiringRatet,1)&&posObjects(i,1)+v>0&&posObjects(i,1)+v<size(sumFiringRatet,2)
                        if ~isnan(sumFiringRatet(bROI(4)-posObjects(i,2)+u,posObjects(i,1)+v))
                            sumFiringRateObject(1,i) =sumFiringRateObject(1,i)+sumFiringRatet(bROI(4)-posObjects(i,2)+u,posObjects(i,1)+v); % change from sunFiringRate to sumFiringRate_conv
                            sum_num=sum_num+1;
                        end
                    end
                end
