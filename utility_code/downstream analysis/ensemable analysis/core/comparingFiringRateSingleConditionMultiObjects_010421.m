function [sumFiringRate_conv,sumFiringRateObject] = comparingFiringRateSingleConditionMultiObjects_010421(firingRate,countTime,objpos,binsize,behavROI)
    % 010421: use sumFiringRate_conv to grab firing rate
                ...
                for v=-obj_range:obj_range
                    if bROI(4)-posObjects(i,2)+u>0&&bROI(4)-posObjects(i,2)+u<size(sumFiringRate_conv,1)&&posObjects(i,1)+v>0&&posObjects(i,1)+v<size(sumFiringRate_conv,2)
                        if ~isnan(sumFiringRate_conv(bROI(4)-posObjects(i,2)+u,posObjects(i,1)+v))
                            sumFiringRateObject(1,i) =sumFiringRateObject(1,i)+sumFiringRate_conv(bROI(4)-posObjects(i,2)+u,posObjects(i,1)+v);
                            sum_num=sum_num+1;
                        end
                    end
                end
