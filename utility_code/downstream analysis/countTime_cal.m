function countTime=countTime_cal(behavpos,behavtime,binsize,idx,behavROI)

% behavROI=[0 0 max(behavpos(:,1)) max(behavpos(:,2))];
behavROI(1:2)=[0 0];
% behavROI=round(behavROI/binsize);
...
%                 countTime(idxxi,idyyi) = countTime(idxxi,idyyi)-(behavtime(j)-behavtime(j-1));
%             end
            if ~isempty(idx) && idx(j)==0 % added 061219
                countTime(idxxi,idyyi) = countTime(idxxi,idyyi)-(behavtime(j)-behavtime(j-1));
            end
        else
            ts = j;
