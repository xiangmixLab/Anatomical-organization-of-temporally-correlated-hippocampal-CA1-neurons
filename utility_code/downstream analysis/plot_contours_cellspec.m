function [CC,jsf,cm] = plot_contours_cellspec(Aor,Cn,thr,display_numbers,max_number,Coor, ln_wd, ln_col,crange)
...
%             plot(cont(1,1:end),cont(2,1:end),'Color',cmap(i+size(Aor,2),:), 'linewidth', ln_wd); hold on;
            plot(cont(1,1:end),cont(2,1:end),'Color','r', 'linewidth', ln_wd); hold on;
        end
    end
else
...
%             CC{i} = contour(reshape(A_temp,d1,d2),[0,0]+A_temp(ind(ff)),'LineColor',cmap(i+size(Aor,2),:), 'linewidth', ln_wd);
            CC{i} = contour(reshape(A_temp,d1,d2),[0,0]+A_temp(ind(ff)),'LineColor','r', 'linewidth', ln_wd);            
...
if display_numbers
    ...
    text((cm(crange,2)),(cm(crange,1)),lbl(crange),'color',[0,0,0],'fontsize',16,'fontname',fontname,'fontweight','bold');
end
axis off;
if ~(nargin < 6 || isempty(Coor))
    jsf = [];
