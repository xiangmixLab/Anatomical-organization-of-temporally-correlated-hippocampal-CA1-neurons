function [all_corr_intra,all_corr_intra_border,all_corr_inter_border]=cluster_crossCorr_border(group,foldername)
...
% 3. cross correlation2: pearson correlation, intra-inter boundary neurons
all_corr_intra={};
all_corr_intra_border={};
all_corr_inter_border={};
for i=1:size(group,1)
    load([foldername{i},'\','neuronIndividuals_new.mat']);
    for j=1:size(group,2)
            ...
            for k1=1:size(nC1_all,1)-1
                for k2=k1+1:size(nC1_all,1)
                    ...
                    if sum((cen1-cen2).^2)^0.5<=neighboor_dis % only pick the pairs with distance smaller than neighbor_dis
                        cef=corrcoef(nC1_all(k1,:),nC1_all(k2,:));
                        all_xcorr(ctt,:)=cef(2);
                        ctt=ctt+1;
                    end
                end
        ...
        for k=1:size(all_nei_bd_CellIdx{i,j},1)
            ...
            for k1=1:size(nC1_border,1)-1
                for k2=k1+1:size(nC1_border,1)
                    ...
                    if sum((cen1-cen2).^2)^0.5<=neighboor_dis
                        cef=corrcoef(nC1_border(k1,:),nC1_border(k2,:));
                        all_xcorr_border(ctt,:)=cef(2);
                        ctt=ctt+1;
                    end
                end
            end
            ...
            for k1=1:size(nC2_border,1)-1
                for k2=k1+1:size(nC2_border,1)
                    ...
                    cen2=cen_curGp2(k2,:);
                    if sum((cen1-cen2).^2)^0.5<=neighboor_dis
                        cef=corrcoef(nC2_border(k1,:),nC2_border(k2,:));
                        all_xcorr_border(ctt,:)=cef(2);
                        ctt=ctt+1;
                    end
                end
            ...
            for k1=1:size(nC1_close,1)
                for k2=1:size(nC2_close,1)
                    cef=corrcoef(nC1_close(k1,:),nC2_close(k2,:));
                    all_xcorr_border_close(ctt,:)=cef(2);
                    ctt=ctt+1;
                end
            end
        ...
        all_corr_intra{i,j}=all_xcorr;
        all_corr_intra_border{i,j}=all_xcorr_border;
        all_corr_inter_border{i,j}=all_xcorr_border_close;
    end
end
