...
function [boundary_all,region_all,region_nnum_all,nd]=cluster_region_cal_021722(group1,session,neuronIndividuals_new,neighboor_dis_given,varargin)
...
%     neighboor_dis(gp)=quantile(neighboor_dis_all{gp},0.7);
end
neighboor_dis=quantile(cell2mat(neighboor_dis_all),0.7);
...
for gp=1:length(ugp)
    ...
    cen=round(neuronIndividuals_new{session}.centroid(group1==ugp(gp),:));
...
for gp=1:length(ugp)
    for i=1:size(boundary_all,2)
        if ~isempty(boundary_all{gp,i})
            ...
            if ~isempty(varargin)&&(isequal(varargin{1},'2p')||isequal(varargin{1},'special'))
