...
function [decoded_position,ori_position_bin,decoded_accuracy,decoded_error,ori_position_bin_nsmooth,decoded_position_nsmooth]=Baysian_decode_2D_position_pipeline(neuron_train,behav_train,neuron_test,behav_test,binsize,cell_list)
...
for i=1:size(neuron_train.C,1)
    [binarized_trace_tr(:,i),~,ntrace] = extract_binary_nC(neuron_train.C(i,:), sampling_frequency, 1);
    [binarized_trace_ts(:,i)] = extract_binary_nC(neuron_test.C(i,:), sampling_frequency, 1);
end
...
min_speed_threshold = 10; % 10mm/sec
...
decoded_position_nsmooth=floor(decoded_positiont);
decoded_position_nsmooth=fillmissing(decoded_position_nsmooth,'nearest');

decoded_positiont=fillmissing(decoded_positiont,'linear');
decoded_position=smoothdata(decoded_positiont,'SmoothingFactor',0.5);

ori_position_bin_nsmooth=floor(interp_behav_vec_ts/binsize);
ori_position_bin_nsmooth=fillmissing(ori_position_bin_nsmooth,'nearest');

ori_position_bin=smoothdata(floor(interp_behav_vec_ts/binsize),'SmoothingFactor',0.05);
ori_position_bin=fillmissing(ori_position_bin,'linear');

decoded_accuracy_t=corrcoef(ori_position_bin(:,1),decoded_position(:,1))+corrcoef(ori_position_bin(:,2),decoded_position(:,2));
decoded_accuracy=decoded_accuracy_t(2)/2; % avg accuracy from x and y
decoded_error=nanmean(sum((ori_position_bin-decoded_position).^2,2).^0.5);