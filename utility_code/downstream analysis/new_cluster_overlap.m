...
% 
function [overlap_idx_avgClust,overlap_idx,overlap_idx_perClust]=new_cluster_overlap(c1,c2)
...
for i=1:size(C,1)
    ...
    if sum(C(i,:))<2
        clust_pairs(i,1)=1; % to avoid divide by 0
    end
end

overlap_idx_avgClust=mean(sum(together_pairs,2)./clust_pairs); % overlap for each cluster, then take average
overlap_idx=sum(together_pairs(:))/nchoosek(length(c1),2); % overlap calculated by all overlap pairs / all possible pairs
overlap_idx_perClust=sum(together_pairs,2)./clust_pairs;
