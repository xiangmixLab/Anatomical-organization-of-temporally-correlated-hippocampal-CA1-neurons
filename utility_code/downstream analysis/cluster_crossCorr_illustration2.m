function cluster_crossCorr_illustration2(group,foldername,miceIdx,condIdx,neiIdx,clust1,clust2)
...
idx1=find(group{miceIdx,1}==clust1);
dist=squareform(pdist(cen(idx1,:)));
dist(dist==0)=inf;
idx1(min(dist,[],2)>neighboor_dis)=[];

idx2=find(group{miceIdx,1}==clust2);
dist=squareform(pdist(cen(idx2,:)));
dist(dist==0)=inf;
idx2(min(dist,[],2)>neighboor_dis)=[];

gp=group{miceIdx,condIdx};
gp(setdiff([1:length(gp)],[idx1;idx2]))=-1;
gp(idx1)=1;
gp(idx2)=2;
...
idx1=all_nei_bd_CellIdx{miceIdx,1}{neiIdx,1};
dist=squareform(pdist(cen(idx1,:)));
dist(dist==0)=inf;
idx1(min(dist,[],2)>neighboor_dis)=[];

idx2=all_nei_bd_CellIdx{miceIdx,1}{neiIdx,2};
dist=squareform(pdist(cen(idx2,:)));
dist(dist==0)=inf;
idx2(min(dist,[],2)>neighboor_dis)=[];
...
idx1=unique(all_nei_bd_closeIdx{miceIdx,1}{neiIdx}(:,1));
idx2=unique(all_nei_bd_closeIdx{miceIdx,1}{neiIdx}(:,2));
