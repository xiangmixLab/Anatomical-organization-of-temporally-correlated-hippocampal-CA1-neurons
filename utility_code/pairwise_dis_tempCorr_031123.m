...
function [all_dis_corr_circle_all_intra,all_dis_corr_circle_all_inter,all_dis_corr_circle_all_all,midx,f,gof]=pairwise_dis_tempCorr_031123(foldername,group1,session)
...
for k=1:length(foldername)
    ...
    if ~isfield(neuronIndividuals_new{session},'imageSize')
        neuronIndividuals_new{session}.imageSize=size(neuronIndividuals_new{session}.Cn);
    end
