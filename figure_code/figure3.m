...
for i=1:size(group_ori_AI163_sameNum_allMice,1)
    ...
    dat_1(i,1)=new_cluster_overlap_latest(gp{1},gp{3});
    dat_1(i,2)=new_cluster_overlap_latest(gp{4},gp{6});
    dat_1(i,3)=new_cluster_overlap_latest(gp{4},gp{5});
    dat_1(i,4)=new_cluster_overlap_latest(gp{5},gp{6});

    for j=1:shuffle
        g1=gp{1}(randperm(length(gp{1})));
        g2=gp{4}(randperm(length(gp{4})));
        g3=gp{5}(randperm(length(gp{5})));
        g4=gp{6}(randperm(length(gp{6})));
        dat_shuf(i,1,j)=new_cluster_overlap_latest(g1,g2);
        dat_shuf(i,2,j)=new_cluster_overlap_latest(g2,g4);
        dat_shuf(i,3,j)=new_cluster_overlap_latest(g2,g3);
        dat_shuf(i,4,j)=new_cluster_overlap_latest(g3,g4);
    end
end
